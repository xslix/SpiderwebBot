...
def call(message: telebot.types.Message):
    ...
    if called is None:
        bot.send_message(message.chat.id,
                         ...
                         parse_mode="html")
        return
    call = Call.get_or_none(Call.called_id == called.chat_id or Call.caller_id == called.chat_id)
    if call is not None:
    ...
    keyboard = types.InlineKeyboardMarkup()
    keyboard.add(types.InlineKeyboardButton(text="Принять", callback_data=f"call take"))
...
def drop_call(message: telebot.types.Message, forced_id=0):
    ...
    call = Call.get_or_none(Call.called_id == message.from_user.id or Call.caller_id == message.from_user.id)
    if call is None:
...
def answer_call(message: telebot.types.Message, forced_id=0):
    if forced_id != 0:
        message.from_user.id = forced_id
    call = Call.get_or_none(Call.called_id == message.from_user.id and Call.is_online == False)
    if call is None:
        bot.send_message(message.from_user.id,
    ...
    bot.send_message(call.caller_id,
                     f"Абонент поднял трубку. Звонок активен. Потрать 100$. Используй /drop /сбросить, чтобы завершить звонок.",
                     parse_mode="html")
    bot.send_message(call.caller_id,
...
def talk(message: telebot.types.Message):
    player = Player.get_or_none(Player.chat_id == message.chat.id)
    call = Call.get_or_none(Call.caller_id == message.from_user.id)
    if call is None:
        call = Call.get_or_none(Call.called_id == message.from_user.id)
    if call is None:
        return
    chat_id = call.caller_id if call.called_id == message.from_user.id else call.called_id
